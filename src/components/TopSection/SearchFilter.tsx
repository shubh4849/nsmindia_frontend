    ...
    <div className="flex items-center space-x-3">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" size="sm" className="bg-surface hover:bg-surface-hover border-border">
            {sortOrder === 'asc' ? <SortAsc size={16} /> : <SortDesc size={16} />}
            <span className="ml-2 capitalize">{sortBy}</span>
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="bg-surface border-border shadow-lg">
          <DropdownMenuItem
